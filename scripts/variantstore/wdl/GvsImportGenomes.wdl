...
task CheckForDuplicateData {
  ...
  command <<<
    ...
    id_list="~{sep='|' sample_names}"
    sample_list=$(grep -E $id_list ~{sample_map} | cut -f1 -d',' | sed -e "s/\(.*\)/'\1'/" | sed -e ':a' -e 'N' -e '$!ba' -e 's/\n/,/g')
    ...
    if [ $BQ_SHOW_RC -eq 0 ]; then
      bq --location=US --project_id=~{project_id} query --use_legacy_sql=false  "SELECT partition_id FROM ${INFO_SCHEMA_TABLE} WHERE table_name like 'pet_%' AND partition_id IN (${sample_list})" > results
    fi
